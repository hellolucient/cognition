    ...
    if (isOpen) {
      ...
      const timer = setTimeout(() => {
        ...
        if (button && !button.getAttribute('data-converted')) {
          const bookmarkletCode = `(function(){try{console.log('=== vanwinkle BOOKMARKLET v4 ENHANCED SELECTORS ===');console.log('URL:',window.location.href);console.log('Document ready state:',document.readyState);var platform='Unknown';var platformEmoji='🤖';if(window.location.href.includes('chatgpt.com')||window.location.href.includes('chat.openai.com')){platform='ChatGPT';platformEmoji='🤖';}else if(window.location.href.includes('claude.ai')){platform='Claude';platformEmoji='🤖';}else if(window.location.href.includes('perplexity.ai')){platform='Perplexity';platformEmoji='🤖';}else if(window.location.href.includes('grok.com')){platform='Grok';platformEmoji='🤖';}else if(window.location.href.includes('g.co/gemini')||window.location.href.includes('gemini.google.com')){platform='Gemini';platformEmoji='🤖';}else if(window.location.href.includes('copilot.microsoft.com')){platform='Copilot';platformEmoji='🤖';}console.log('Detected platform:',platform);

// Smart citation extraction function - uses ChatGPT's exact HTML structure
function extractCitations() {
  var citations = [];
  var citationPills = document.querySelectorAll('[data-testid="webpage-citation-pill"]');
  console.log('Found', citationPills.length, 'citation pills');
  
  citationPills.forEach(function(pill, index) {
    try {
      // Extract source name from the first span
      var sourceSpan = pill.querySelector('span');
      var sourceName = sourceSpan ? sourceSpan.textContent.trim() : '';
      
      // Extract number from the rounded span (the +number part)
      var numberSpan = pill.querySelector('.rounded-full');
      var number = numberSpan ? numberSpan.textContent.trim() : '';
      
      if (sourceName && number) {
        var citation = sourceName + ' ' + number;
        citations.push(citation);
        console.log('Extracted citation', index + 1, ':', citation);
      }
    } catch (err) {
      console.log('Error extracting citation', index + 1, ':', err);
    }
  });
